...
async function dealDetail(id) {
  const deal = await api(`/api/deals/${encodeURIComponent(id)}`); const rev = asArray(deal.economics_revisions)[0] || {};
  ...
  $('#deal-workspace').onsubmit = async e => { e.preventDefault(); const v=Object.fromEntries(new FormData(e.currentTarget)); ['estimated_budget_min','estimated_budget_max'].forEach(k=>v[k]=v[k]?Number(v[k]):null); try { v.decision_makers=JSON.parse(v.decision_makers||'[]'); await api(`/api/deals/${id}`,{method:'PATCH',body:v}); toast('Сделка сохранена'); await dealDetail(id); } catch(err) { toast(err.message); } };
}
