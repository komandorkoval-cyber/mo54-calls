...
function insightMarkup(call, insightData) {
  ...
  if (isNovofon(call) && call.automation?.analysis === 'disabled' && !insightData.summary) {
    return '<p class="muted">Транскрипт сохранён. ИИ-анализ выключен до отдельного разрешения на передачу текста.</p>';
  }
...
async function callDetail(id) {
  ...
  const showRetryActions = !isNovofon(call) || call.transcription_available === true || call.analysis_available === true || (isNovofon(call) && call.automation?.analysis === 'waiting' && Boolean(call.transcript?.text));
