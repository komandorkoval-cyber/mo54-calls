...
нажмите `Enter` в соответствующей позиции локального аудио: редактор сам
создаст новую реплику, переключит роль и сохранит её временную границу.
`Shift+Enter` оставляет перенос внутри реплики, а `Backspace` в начале новой
реплики отменяет ошибочное разделение. До утверждения проверяются все роли,
порядок и отсутствие пересечений, а также границы реальной длительности
аудио. Исходный ASR-транскрипт остаётся отдельным
локальным эталоном; утверждённая версия и WER/CER/метрики ролей сохраняются
